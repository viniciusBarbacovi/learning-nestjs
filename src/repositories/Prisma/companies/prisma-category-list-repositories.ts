...
import { CategoriesRepositories } from '../../companies/list-category-repositories';

@Injectable()
export class PrismaCategoriesRepository implements CategoriesRepositories {
  constructor(private readonly prisma: PrismaService) {}

  async listByCompany(companyId: string, userId: string): Promise<{ id: string; name: string }[]> {
    // Verifica se o usuário é membro da empresa (OWNER ou EMPLOYEE)
    const membership = await this.prisma.companyMember.findUnique({
      where: { userId_companyId: { userId, companyId } },
    });

    if (!membership) throw new ForbiddenException('User does not belong to this company');
